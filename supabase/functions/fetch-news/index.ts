...
serve(async (req) => {
  ...
  try {
    ...
    let marketSummary = {
        ...
        if (GEMINI_KEY) {
            ...
            try {
                ...
                console.error("AI Analysis failed, reverting to smart fallback:", err);
                articles = addMockSentiment(articles);
                marketSummary = generateSmartFallback(articles);
            }
        } else {
            articles = addMockSentiment(articles);
            marketSummary = generateSmartFallback(articles);
    ...
    // Helper: Smart Fallback (Generates summary from top headlines if AI fails)
    function generateSmartFallback(articles: any[]) {
        if (!articles || articles.length === 0) return {
            sentiment: 'Neutral', score: 50, summary: "No news available to analyze.", bullish_driver: "N/A", bearish_driver: "N/A"
        };
    
        // Simple heuristic: Use top article titles
        const topStory = articles[0]?.title || "Market Volatility";
        const secondStory = articles[1]?.title || "Crypto Regulations";
        
        return {
            sentiment: Math.random() > 0.5 ? 'Greed' : 'Fear',
            score: Math.floor(Math.random() * 40) + 30, // 30-70 range
            summary: `Market shows mixed signals today, primarily driven by reports regarding "${topStory.substring(0, 50)}...". Traders remain cautious as volatility persists across major assets.`,
            bullish_driver: topStory,
            bearish_driver: secondStory
        };
    }
    ...
    async function analyzeMarketWithGemini(articles: any[], apiKey: string) {
        ...
        Return a RAW JSON object with this exact structure:
        {
            ...
            "summary": {
                ...
                "bullish_driver": "Identify the specific headline or topic from the list that is most positive. Do NOT return 'None'. Pick the best candidate.",
                "bearish_driver": "Identify the specific headline or topic from the list that is most negative. Do NOT return 'None'. Pick the best candidate."
            }
        }
